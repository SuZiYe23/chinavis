...
========================================= */
import { DataStore, getRelationsByOpera, getRolesFromRolesFile, normalizeType } from '../dataLoader.js'
import { FONT_DISPLAY, FONT_BODY, goldenTooltip, ROLE_COLORS, FALLBACK, emptyTitle, bindResize, COLORS, SERIES_COLORS } from '../chartTheme.js'
...
export function renderRelationNetwork (operaType = '全部') {
  let operas = DataStore.operaCompact || []
  if (operaType !== '全部') operas = operas.filter(o => normalizeType(o.opera_type) === operaType)
  currentOperaList = operas.filter(o => {
    const rel = getRelationsByOpera(o.opera_name)
...
function renderDim (dim, nodes, links, rawData) {
  ...
  else if (dim === 'narrative') renderNarrativeView(_rightChart, nodes, links)
}
...
function renderStructureView (chart) {
  ...
  chart.setOption({
    backgroundColor: 'transparent', textStyle: { fontFamily: FONT_BODY },
    tooltip: { backgroundColor: COLORS.tooltipBg, borderColor: COLORS.tooltipBorder, borderWidth: 2, padding: [10, 14], textStyle: { color: COLORS.textLight, fontSize: 12 }, extraCssText: 'border-radius:6px;box-shadow:0 6px 24px rgba(0,0,0,.4);', formatter: function (params) {
      if (!params || !params.value) return ''
      var h = '<div style="font-weight:bold;color:#ffd27f;font-family:' + FONT_DISPLAY + ';margin-bottom:4px;">' + params.seriesName + '</div>'
      params.value.forEach(function (v, i) { h += '<div style="display:flex;justify-content:space-between;gap:20px;line-height:1.8;"><span>' + indicator[i].name + '</span><span style="font-weight:bold;">' + v + '</span></div>' })
      return h
    }},
    legend: { type: 'scroll', top: 0, right: 0, textStyle: { color: COLORS.legendText, fontSize: 18 }, pageTextStyle: { color: COLORS.legendText }, itemWidth: 10, itemHeight: 8, itemGap: 6 },
    radar: { center: ['50%', '55%'], radius: '85%', indicator: indicator, name: { textStyle: { color: '#c4a882', fontSize: 20 } }, splitArea: { areaStyle: { color: ['rgba(100,25,20,.1)', 'rgba(100,25,20,.05)'] } }, splitLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } }, axisLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } } },
    series: [{ type: 'radar', data: metrics.map(function (m, i) { return { name: m.type, value: [m.avgDegree, m.avgDensity, m.avgComponents, m.avgEdgeCount], lineStyle: { color: palette[i % palette.length], width: 2 }, areaStyle: { color: palette[i % palette.length], opacity: 0.06 }, itemStyle: { color: palette[i % palette.length] } } }) }]
  })
  bindResize(chart)
}
...
function renderRoleView (chart) {
  ...
  chart.setOption({
    ...
    legend: { type: 'scroll', top: 0, right: 0, textStyle: { color: COLORS.legendText, fontSize: 18 }, pageTextStyle: { color: COLORS.legendText }, itemWidth: 10, itemHeight: 8, itemGap: 6 },
    radar: { center: ['50%', '55%'], radius: '85%', indicator: indicator, name: { textStyle: { color: '#c4a882', fontSize: 20 } }, splitArea: { areaStyle: { color: ['rgba(100,25,20,.1)', 'rgba(100,25,20,.05)'] } }, splitLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } }, axisLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } } },
    series: [{ type: 'radar', data: metrics.map(function (m, i) { return { name: m.type, value: [m.corePct, m.diversity, m.highImpPct], lineStyle: { color: palette[i % palette.length], width: 2 }, areaStyle: { color: palette[i % palette.length], opacity: 0.06 }, itemStyle: { color: palette[i % palette.length] } } }) }]
  })
  bindResize(chart)
}
...
function computeRelationMetrics () {
  ...
  DataStore.operaRelations.forEach(function (item) {
    ...
    typeMap[type].intensities.push(Math.min(1, totalInt / Math.max(1, pos + neg) / 10))
  })
...
function renderRelationView (chart) {
  ...
  chart.setOption({
    ...
    legend: { type: 'scroll', top: 0, right: 0, textStyle: { color: COLORS.legendText, fontSize: 18 }, pageTextStyle: { color: COLORS.legendText }, itemWidth: 10, itemHeight: 8, itemGap: 6 },
    radar: { center: ['50%', '55%'], radius: '85%', indicator: indicator, name: { textStyle: { color: '#c4a882', fontSize: 20 } }, splitArea: { areaStyle: { color: ['rgba(100,25,20,.1)', 'rgba(100,25,20,.05)'] } }, splitLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } }, axisLine: { lineStyle: { color: 'rgba(255,220,180,.08)' } } },
    series: [{ type: 'radar', data: metrics.map(function (m, i) { return { name: m.type, value: [m.posNegRatio, m.relRichness, m.avgIntensity], lineStyle: { color: palette[i % palette.length], width: 2 }, areaStyle: { color: palette[i % palette.length], opacity: 0.06 }, itemStyle: { color: palette[i % palette.length] } } }) }]
  })
  bindResize(chart)
}

/* 4. 叙事维度 — 柱状图 */
function renderNarrativeView (chart, nodes, links) {
  if (!nodes.length) { chart.setOption({ backgroundColor: 'transparent', ...emptyTitle('暂无数据') }); return }
  var degMap = {}
  links.forEach(function (l) { degMap[l.source] = (degMap[l.source] || 0) + 1; degMap[l.target] = (degMap[l.target] || 0) + 1 })
  var n = nodes.length
  var core = Object.values(degMap).filter(function (d) { return d >= 5 }).length
  var peri = Math.max(1, n - core)
  var coreLinks = links.filter(function (l) { var s = degMap[l.source] || 0; var t = degMap[l.target] || 0; return s >= 5 || t >= 5 }).length
  var periLinks = links.length - coreLinks
  var coreRatio = core > 0 ? +(coreLinks / core).toFixed(1) : 0
  var periRatio = peri > 0 ? +(periLinks / peri).toFixed(1) : 0
  var cpRatio = periRatio > 0 ? +(coreRatio / periRatio).toFixed(2) : coreRatio
  var degs = Object.values(degMap)
  var avgDeg = degs.length > 0 ? degs.reduce(function (s, v) { return s + v }, 0) / degs.length : 1
  var stdDeg = degs.length > 1 ? Math.sqrt(degs.reduce(function (s, v) { return s + Math.pow(v - avgDeg, 2) }, 0) / degs.length) : 1
  var balance = +(avgDeg / Math.max(stdDeg, 0.01) * 10).toFixed(1)

  chart.setOption({
    backgroundColor: 'transparent', textStyle: { fontFamily: FONT_BODY },
    tooltip: { trigger: 'axis', backgroundColor: COLORS.tooltipBg, borderColor: COLORS.tooltipBorder, borderWidth: 2, padding: [10, 14], textStyle: { color: COLORS.textLight, fontSize: 11 }, extraCssText: 'border-radius:6px;box-shadow:0 6px 24px rgba(0,0,0,.4);' },
    series: [{
      type: 'bar', barWidth: '45%',
      data: [
        { value: Math.min(cpRatio, 20), itemStyle: { color: '#b23a2a', borderRadius: [3,3,0,0] } },
        { value: Math.min(coreRatio, 20), itemStyle: { color: '#d4954a', borderRadius: [3,3,0,0] } },
        { value: Math.min(periRatio, 20), itemStyle: { color: '#8a7a6a', borderRadius: [3,3,0,0] } },
        { value: Math.min(balance, 20), itemStyle: { color: '#3d6f94', borderRadius: [3,3,0,0] } }
      ],
      label: { show: true, position: 'top', color: '#c4a882', fontSize: 9, formatter: function (p) { return p.value >= 20 ? p.value + '+' : '' + p.value } },
      emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(255,200,100,0.2)' } }
    }]
  })
  bindResize(chart)
}

export function prevOpera () {
  if (currentOperaList.length === 0) return
  currentIndex = (currentIndex - 1 + currentOperaList.length) % currentOperaList.length
  renderCurrentOpera()
}

export function nextOpera () {
  if (currentOperaList.length === 0) return
